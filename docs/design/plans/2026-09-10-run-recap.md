...
- **Build with a `dotnet` that has the .NET 9 SDK.** If the one first on PATH doesn't (an old x86 install can shadow it), call the right one by its full path.
...
Run:
```powershell
dotnet new sln -n RunRecap
dotnet sln RunRecap.sln add src\RunRecap\RunRecap.csproj
dotnet build src\RunRecap\RunRecap.csproj -c Release --nologo -v q
```
...
Run: `dotnet run --project tests\RunRecap.Tests -- Attribution`
...
```powershell
dotnet sln RunRecap.sln add tests\RunRecap.Tests\RunRecap.Tests.csproj
```
...
Run: `dotnet run --project tests\RunRecap.Tests -- RunStats`
...
Run: `dotnet run --project tests\RunRecap.Tests -- Store`
...
Run: `dotnet run --project tests\RunRecap.Tests -- RecapBuilder`
