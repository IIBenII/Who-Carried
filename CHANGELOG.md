...
- **Share run:** `"shareUrl"` in `settings.json` posts the recap to a site when a run ends, and from the **Share run** button. The page link is copied. **Export as image** is unchanged.
