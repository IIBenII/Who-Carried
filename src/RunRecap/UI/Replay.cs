...
/// Pictures\Run Recap, then closes. Inert otherwise.
/// </summary>
internal static class Replay
{
    ...
    {
        string folder = Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyPictures), "Run Recap");
        string path = Path.Combine(folder, $"run-{date.ToString("yyyy-MM-dd_HHmm", CultureInfo.InvariantCulture)}-{(victory ? "victory" : "defeat")}.png");
