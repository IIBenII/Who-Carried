...
internal static class SummaryCard
{
    ...
    /// <summary>The game's top bar: "Run recap · Victory", floor, time, ascension, team damage, and the date.</summary>
    private static Control TopBar(Kit k, RecapView view, DateTime date)
    {
        ...
        title.AddChild(Kit.Center(k.Strong("Run recap · ", 32)));
    ...
    private static Control Footer(Kit k, RecapView view)
    {
        ...
        row.AddChild(Kit.Fill());
        row.AddChild(k.Text("Run Recap for Slay the Spire 2", 13, RecapTheme.Faint));
        return Pull(row, -4);
    }
}
